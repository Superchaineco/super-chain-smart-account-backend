  ...
  async getValue(eoas: string[]): Promise<number> {
    ...
    const fetchFunction = async () => {
      const transactions = eoas.reduce(async (accPromise, eoa) => {
        const response = await axios.get(`https://api.routescan.io/v2/network/mainnet/evm/34443/etherscan/api?apikey=${ROUTESCAN_API_KEY}&module=account&action=txlist&address=${eoa}&startblock=0&endblock=99999999&page=1&offset=51&sort=asc`)

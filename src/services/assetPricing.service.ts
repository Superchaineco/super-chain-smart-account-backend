    ...
    const fetchFunction = async () => {
        try {
            const response = await axios.get(
                `https://api.coingecko.com/api/v3/simple/token_price/optimistic-ethereum?contract_addresses=${assetAddress}&vs_currencies=usd`,
                {
                    headers: {
            ...
            console.log('Coingecko price data:', response.data)
